# Arria II Constraint file.

# PCI_CF_Device_Family = Arria II
# PCI_CF_Bus_Speed = 33.33MHz, 66.67 MHz 
# PCI_CF_Core_Support = MT64, T64, MT32, T32
# PCI_CF_Revision_No = 6.1   
# PCI_CF_Date_Of_Creation (MM/DD/YY)= 07/25/08
...
proc sanity_checks { speed device core  } {
        ...
        # Check if customer is targeting on Arria II device, LPS
        set device_family [lindex [get_part_info -family $device] 0] 
        if { $device_family == "Arria II GX" } {
        } else {
                puts "Error: This constraint file is specific to Arria II GX device family."
                puts "       You are not targeting an Arria II GX device."
	...
	set package [get_part_info -package $device]
	puts "PCI Info: Pin count is $pin_count"
        
	if { ($core == "MT64" || $core == "T64") && ($speed == "66") && ($pin_count < 404) } {
        	puts "PCI Error: Altera does not support the pci_mt64 and pci_t64 function in CU17 packages for 66 MHz PCI"
                puts "       Change the Arria II GX device package in your project"
                error ""
        }
	
	if { $package == "UFBGA" } {
		puts "PCI Error: $package is not supported. Please choose other package."
		error ""
        }

        if { ($pin_count < 208) } {
                puts  ""
                puts  "PCI Error :	The selected devices pin counts is too small and is not supported"
                error "PCI Error :	Please select devices that have higher pin counts"
        }    
        
        #if { ($device_speed == "5" || $device_speed == "6") && $speed == "66" } {
        #        puts "PCI Error: Altera does not support 66 MHz PCI in Arria II GX -5 and -6 device speed grades"
...
proc pci_pin_asgn {device full_64 master core pin_list no_pins got_prefix got_suffix prefix suffix use_dedicated_clk } {
        ...
        # Container constraints for Arria II
...
proc generate_pci_sdc { pin_list pci_timing master hierarchy core core_lower full_64 } {
	...
	foreach bidir $bidir_pin {
	       if { [regexp "$ad" $bidir] } {
		     ...
		     puts $sdc_file "###############################"
		     puts $sdc_file "set_output_delay -clock PCI_CLOCK $output_delay_value_tco \[\get_ports $ad\[*\]]"
		     ...
		     puts $sdc_file "###############################"
		     puts $sdc_file "set_output_delay -clock PCI_CLOCK $output_delay_value_tco \[\get_ports $cben\[*\]]"
		   ...
		   puts $sdc_file "###############################"
		   puts $sdc_file "set_output_delay -clock PCI_CLOCK $output_delay_value_tco \[\get_ports $bidir]"
	...
	foreach output $output_pin {
		...
		if { [regexp ".*reqn.*" $output] } {
			puts $sdc_file "set_output_delay -clock PCI_CLOCK $output_delay_value_tco \[\get_ports $output]"
		} else {
			puts $sdc_file "set_output_delay -clock PCI_CLOCK $output_delay_value_tco \[\get_ports $output]"
		}        
	}
...
puts "Info: You have sourced the Arria II GX PCI constraint file."
