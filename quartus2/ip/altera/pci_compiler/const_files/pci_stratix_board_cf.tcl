...
proc sanity_checks { speed device core  } {
        ...
        if { $device == "AUTO" } {
                puts "Error: Cannot set DEVICE to AUTO. Assign"
                puts "       EP1S25F1020C5 if you are using Stratix PCI Kit, or"
                puts "       EP1S60F1020C6 if you are using Stratix Profesional Kit"
                error ""
        }
        ...
        if { $device == "EP1S25F1020C5" || $device == "EP1S60F1020C6" } {
        } else {
                puts "Error: You are not using the Stratix board devices. Assign"
                puts "       EP1S25F1020C5 if you are using Stratix PCI Kit, or"
                puts "       EP1S60F1020C6 if you are using Stratix Profesional Kit"
                error ""
        }
...
proc pci_pin_asgn { device full_64 master core pin_list no_pins got_prefix got_suffix prefix suffix  } {
        ...
        # get device without speed grade
...
proc get_pinout_group_name { device_no_speed } {
        ...
        # array name == pin
        array set pin { EP1S25F1020     S25_F1020}
        array set pin { EP1S60F1020     S60_F1020}
...
# Note:
# The pin outs information in this document is intended for 
# Stratix PCI Kit and Stratix PCI Profesional Kit only.
...
proc generate_pci_sdc { pin_list pci_timing master hierarchy core core_lower full_64 } {
	...
	puts $sdc_file "set_input_delay -clock PCI_CLOCK -max 0 $rstn"
	puts $sdc_file "set_input_delay -clock PCI_CLOCK -min 100 $rstn"
	foreach bidir $bidir_pin {
	       if { [regexp "$ad" $bidir] } {
...
puts "Info: You have sourced the Stratix Board PCI constraint file."
