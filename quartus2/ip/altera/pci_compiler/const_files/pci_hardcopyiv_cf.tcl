...
proc sanity_checks { speed device core  } {
	...
	# Check if customer is targeting on HardCopy IV device
	set device_family [lindex [get_part_info -family $device] 0]        

	if { $device_family == "HardCopy IV" } {
	} else {
			puts "Error: This constraint file is specific to HardCopy IV device family."
			puts "       You are not targeting a HardCopy IV device."
	...
	#		puts "       Change the HardCopy IV device package in your project"
	#		error ""
	#}    
...
puts "Info: You have sourced the HardCopy IV PCI constraint file."
