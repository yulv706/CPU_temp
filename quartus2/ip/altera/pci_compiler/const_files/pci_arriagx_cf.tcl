# Arria GX Constraint file.

# PCI_CF_Device_Family = Arria GX
...
#   #add_pci_constraints [-speed 33] [-no_compile] [-no_pinouts] [-pin_prefix <pin_prefix_name>] [-pin_suffix <pin_suffix_name>] [-help]  
            ...
            0 { 
                switch -exact -- $arg {
                    ...
                    -help { 
                        ...
                        puts "Usage"
                        puts "\t add_pci_constraints \[-speed 33 \] \[-no_compile\] \[-no_pinouts\] \[-pin_prefix <pin_prefix_name>\] \[-pin_suffix <pin_suffix_name>\] \[-help\]\n"
...
proc sanity_checks { speed device core  } {
    ...
    # Check if customer is targeting on Arria GX device, EP2S
    set device_family [lindex [get_part_info -family $device] 0]        
    
    if { $device_family == "Arria GX" } {
    } else {
            puts "Error: This constraint file is specific to Arria GX device family."
            puts "       You are not targeting a Arria GX device."
    ...
    if { ($core == "MT64" || $core == "T64") && ($speed == "66") && ($pin_count == "484" || $pin_count == "672") } {
            ...
            puts "       Change the Arria GX device package in your project"
            error ""
    }    
    ...
    #        puts "       Change the Arria GX device in your project"
    #        error ""        	
    #}  
}
...
puts "Info: You have sourced the Arria GX PCI constraint file."
