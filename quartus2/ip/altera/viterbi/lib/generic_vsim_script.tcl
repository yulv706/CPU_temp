...
## in order to use the provided VHDL testbench. We don't have Verilog Testbench yet
	...
	vcom -explicit -93 -work vit_work [file join $megacore_lib_dir vi_interface.vhd]
	vcom -explicit -93 -work vit_work [file join $megacore_lib_dir vi_functions.vhd]
	vcom -explicit -93 -work vit_work [file join $megacore_lib_dir vi_bench.vhd]
	vcom -explicit -93 -work vit_work [file join $megacore_lib_dir auk_vit_var_enc_ent.vhd]
	vcom -explicit -93 -work vit_work [file join $megacore_lib_dir auk_vit_var_enc_arc_rtl.vhd]
	if {$use_ipfs == 1} {
		if {[string match "vho" $ext_ipfs]} {
			vcom -explicit -93 -work vit_work [file join $vsim_working_dir ${design_name}.vho]
		} elseif {[string match "vo" $ext_ipfs]} {
			vlog -work vit_work [file join $vsim_working_dir ${design_name}.vo]
		}
	} elseif {$development == 1} {
		if {[string match "vhdl" $hdl_language]} {
			vcom -explicit -93 -work vit_work [file join $vsim_working_dir ${design_name}.vhd]
		} elseif {[string match "verilog" $hdl_language]} {
			vlog -work vit_work [file join $megacore_wrapper_dir ${design_name}.v]
		}
		## Gate-level
	} elseif {$development == 2} {
		if {[string match "vhdl" $hdl_language]} {
			vcom -explicit -93 -work vit_work [file join $vsim_working_dir simulation modelsim ${design_name}.vho]
		} elseif {[string match "verilog" $hdl_language]} {
			vlog -work vit_work [file join $vsim_working_dir simulation modelsim ${design_name}.vo]
		}
	}
	vcom -explicit -93 -work vit_work [file join $megacore_lib_dir Bench_vit_${st}_atl_ent.vhd]
	vcom -explicit -93 -work vit_work [file join $megacore_lib_dir Bench_vit_${st}_atl_arc_ben.vhd]
...
lappend vsim_cmd "vit_work.cfg_testbench"
if {$development == 2} {
...
lappend vsim_cmd "$timing_resolution"
	...
	#vsim -L sgate -L altera_mf -L lpm vit_work.cfg_testbench 
	eval $vsim_cmd
	if {$development == 1} {
