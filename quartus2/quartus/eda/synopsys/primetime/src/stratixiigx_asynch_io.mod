...
MODEL
MODEL_VERSION "1.0";
DESIGN "stratixiigx_asynch_io";

INPUT datain, oe;
INPUT modesel[35:0];
INPUT regin, ddioregin;
INPUT delayctrlin[5:0];
INPUT offsetctrlin[5:0];
INPUT dqsupdateen;
OUTPUT dqsbusout;
...
td_padio_dqsbusout: DELAY (NONINVERTING) padio dqsbusout COND
             /* ( op_mode == input || op_mode == bidir )
             */
            ( (modesel[0] == 1) || (modesel[2] == 1));
...
td_regin_dqsbusout: DELAY (NONINVERTING) regin dqsbusout;

td_delayctrlin_dqsbusout: DELAY  delayctrlin dqsbusout;

td_offsetctrlin_dqsbusout: DELAY offsetctrlin dqsbusout;
