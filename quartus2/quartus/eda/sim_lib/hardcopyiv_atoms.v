...
`endif
   ram_type = 0;
    ...
    // REMHARDCOPYIV dataout_a = 'b0;
    // REMHARDCOPYIV if (mode_is_dp || mode_is_bdp) dataout_b = 'b0;
        dataout_a = 'b0;
...
begin
   if (mode_is_bdp & active_write_clear_b & we_b_reg)
        mem_invalidate = 1'b1;
  else if ((mode_is_dp | mode_is_bdp) & re_b_reg)
    begin
        if (primary_port_is_b)
        begin
            read_data_latch = 'bx;
...
defparam dataout_a_register.width = port_a_data_width;
 ...
 assign portadataout =  (out_a_is_reg) ? dataout_a_reg : (
                            (dataout_a_clr || dataout_a_clr_reg) ? 'b0 : dataout_a
                       );
...
defparam dataout_b_register.width = port_b_data_width;
 ...
 assign portbdataout = (out_b_is_reg) ? dataout_b_reg : (
                          (dataout_b_clr || dataout_b_clr_reg) ? 'b0 : dataout_b
                      );
...
assign portbdataout_tmp = (port_b_data_out_clock == "clock1") ? dataout_b_reg : (dataout_b_clr_in) ? {data_width{1'b0}} : dataout_b;
...
assign abs_a        = dataa_sign ? (~dataa + 1) : dataa;
assign abs_b        = datab_sign ? (~datab + 1) : datab;
...
assign dataa_sload = ena[dataa_clk_value] ? 'b1 : 'b0;
...
assign datab_sload = ena[datab_clk_value] ? 'b1 : 'b0;
...
assign signa_sload = ena[signa_clk_value] ? 'b1 : 'b0;
...
assign signb_sload = ena[signb_clk_value] ? 'b1 : 'b0;
...
//assign the corresponding clk,aclr,enable and bypass register values.
assign scanouta_clk = clk[scanouta_clk_value] ? 'b1 : 'b0;
assign scanouta_aclr = aclr[scanouta_aclr_value] || ~devclrn || ~devpor   ? 'b1 : 'b0;
assign scanouta_sload = ena[scanouta_clk_value] ? 'b1 : 'b0;
assign scanouta_bypass_register = (scanouta_clock == "none") ? 'b1 : 'b0;
...
//assign the corresponding clk,aclr,enable and bypass register values.
assign zeroloopback_clk_ir = clk[zeroloopback_clkval_ir] ? 'b1 : 'b0;
assign zeroloopback_aclr_ir = aclr[zeroloopback_aclrval_ir] || ~devclrn || ~devpor   ? 'b1 : 'b0;
assign zeroloopback_sload_ir = ena[zeroloopback_clkval_ir] ? 'b1 : 'b0;
assign zeroloopback_bypass_register_ir = (zeroloopback_clock == "none") ? 'b1 : 'b0;
...
//assign the corresponding clk,aclr,enable and bypass register values.
assign zeroacc_clk_ir = clk[zeroacc_clkval_ir] ? 'b1 : 'b0;
assign zeroacc_aclr_ir = aclr[zeroacc_aclrval_ir] || ~devclrn || ~devpor   ? 'b1 : 'b0;
assign zeroacc_sload_ir = ena[zeroacc_clkval_ir] ? 'b1 : 'b0;
assign zeroacc_bypass_register_ir = (zeroacc_clock == "none") ? 'b1 : 'b0;
...
//assign the corresponding clk,aclr,enable and bypass register values.
assign signa_clk_ir = clk[signa_clkval_ir] ? 'b1 : 'b0;
assign signa_aclr_ir = aclr[signa_aclrval_ir] || ~devclrn || ~devpor   ? 'b1 : 'b0;
assign signa_sload_ir = ena[signa_clkval_ir] ? 'b1 : 'b0;
assign signa_bypass_register_ir = (signa_clock == "none") ? 'b1 : 'b0;
...
//assign the corresponding clk,aclr,enable and bypass register values.
assign signb_clk_ir = clk[signb_clkval_ir] ? 'b1 : 'b0;
assign signb_aclr_ir = aclr[signb_aclrval_ir] || ~devclrn || ~devpor   ? 'b1 : 'b0;
assign signb_sload_ir = ena[signb_clkval_ir] ? 'b1 : 'b0;
assign signb_bypass_register_ir = (signb_clock == "none") ? 'b1 : 'b0;
...
//assign the corresponding clk,aclr,enable and bypass register values.
assign rotate_clk_ir = clk[rotate_clkval_ir] ? 'b1 : 'b0;
assign rotate_aclr_ir = aclr[rotate_aclrval_ir] || ~devclrn || ~devpor   ? 'b1 : 'b0;
assign rotate_sload_ir = ena[rotate_clkval_ir] ? 'b1 : 'b0;
assign rotate_bypass_register_ir = (rotate_clock == "none") ? 'b1 : 'b0;
...
//assign the corresponding clk,aclr,enable and bypass register values.
assign shiftright_clk_ir = clk[shiftright_clkval_ir] ? 'b1 : 'b0;
assign shiftright_aclr_ir = aclr[shiftright_aclrval_ir] || ~devclrn || ~devpor   ? 'b1 : 'b0;
assign shiftright_sload_ir = ena[shiftright_clkval_ir] ? 'b1 : 'b0;
assign shiftright_bypass_register_ir = (shiftright_clock == "none") ? 'b1 : 'b0;
...
//assign the corresponding clk,aclr,enable and bypass register values.
assign round_clk_ir = clk[round_clkval_ir] ? 'b1 : 'b0;
assign round_aclr_ir = aclr[round_aclrval_ir] || ~devclrn || ~devpor   ? 'b1 : 'b0;
assign round_sload_ir = ena[round_clkval_ir] ? 'b1 : 'b0;
assign round_bypass_register_ir = (round_clock == "none") ? 'b1 : 'b0;
...
//assign the corresponding clk,aclr,enable and bypass register values.
assign saturate_clk_ir = clk[saturate_clkval_ir] ? 'b1 : 'b0;
assign saturate_aclr_ir = aclr[saturate_aclrval_ir] || ~devclrn || ~devpor   ? 'b1 : 'b0;
assign saturate_sload_ir = ena[saturate_clkval_ir] ? 'b1 : 'b0;
assign saturate_bypass_register_ir = (saturate_clock == "none") ? 'b1 : 'b0;
...
//assign the corresponding clk,aclr,enable and bypass register values.
assign roundchainout_clk_ir = clk[roundchainout_clkval_ir] ? 'b1 : 'b0;
assign roundchainout_aclr_ir = aclr[roundchainout_aclrval_ir] || ~devclrn || ~devpor   ? 'b1 : 'b0;
assign roundchainout_sload_ir = ena[roundchainout_clkval_ir] ? 'b1 : 'b0;
assign roundchainout_bypass_register_ir = (roundchainout_clock == "none") ? 'b1 : 'b0;
...
//assign the corresponding clk,aclr,enable and bypass register values.
assign saturatechainout_clk_ir = clk[saturatechainout_clkval_ir] ? 'b1 : 'b0;
assign saturatechainout_aclr_ir = aclr[saturatechainout_aclrval_ir] || ~devclrn || ~devpor   ? 'b1 : 'b0;
assign saturatechainout_sload_ir = ena[saturatechainout_clkval_ir] ? 'b1 : 'b0;
assign saturatechainout_bypass_register_ir = (saturatechainout_clock == "none") ? 'b1 : 'b0;
...
//assign the corresponding clk,aclr,enable and bypass register values.
assign zeroloopback_clk_pip = clk[zeroloopback_clkval_pip] ? 'b1 : 'b0;
assign zeroloopback_aclr_pip = aclr[zeroloopback_aclrval_pip] || ~devclrn || ~devpor   ? 'b1 : 'b0;
assign zeroloopback_sload_pip = ena[zeroloopback_clkval_pip] ? 'b1 : 'b0;
assign zeroloopback_bypass_register_pip = (zeroloopback_pipeline_clock == "none") ? 'b1 : 'b0;
...
//assign the corresponding clk,aclr,enable and bypass register values.
assign zeroacc_clk_pip = clk[zeroacc_clkval_pip] ? 'b1 : 'b0;
assign zeroacc_aclr_pip = aclr[zeroacc_aclrval_pip] || ~devclrn || ~devpor   ? 'b1 : 'b0;
assign zeroacc_sload_pip = ena[zeroacc_clkval_pip] ? 'b1 : 'b0;
assign zeroacc_bypass_register_pip = (zeroacc_pipeline_clock == "none") ? 'b1 : 'b0;
...
//assign the corresponding clk,aclr,enable and bypass register values.
assign signa_clk_pip = clk[signa_clkval_pip] ? 'b1 : 'b0;
assign signa_aclr_pip = aclr[signa_aclrval_pip] || ~devclrn || ~devpor   ? 'b1 : 'b0;
assign signa_sload_pip = ena[signa_clkval_pip] ? 'b1 : 'b0;
assign signa_bypass_register_pip = (signa_pipeline_clock == "none") ? 'b1 : 'b0;
...
//assign the corresponding clk,aclr,enable and bypass register values.
assign signb_clk_pip = clk[signb_clkval_pip] ? 'b1 : 'b0;
assign signb_aclr_pip = aclr[signb_aclrval_pip] || ~devclrn || ~devpor   ? 'b1 : 'b0;
assign signb_sload_pip = ena[signb_clkval_pip] ? 'b1 : 'b0;
assign signb_bypass_register_pip = (signb_pipeline_clock == "none") ? 'b1 : 'b0;
...
//assign the corresponding clk,aclr,enable and bypass register values.
assign rotate_clk_pip = clk[rotate_clkval_pip] ? 'b1 : 'b0;
assign rotate_aclr_pip = aclr[rotate_aclrval_pip] || ~devclrn || ~devpor   ? 'b1 : 'b0;
assign rotate_sload_pip = ena[rotate_clkval_pip] ? 'b1 : 'b0;
assign rotate_bypass_register_pip = (rotate_pipeline_clock == "none") ? 'b1 : 'b0;
...
//assign the corresponding clk,aclr,enable and bypass register values.
assign shiftright_clk_pip = clk[shiftright_clkval_pip] ? 'b1 : 'b0;
assign shiftright_aclr_pip = aclr[shiftright_aclrval_pip] || ~devclrn || ~devpor   ? 'b1 : 'b0;
assign shiftright_sload_pip = ena[shiftright_clkval_pip] ? 'b1 : 'b0;
assign shiftright_bypass_register_pip = (shiftright_pipeline_clock == "none") ? 'b1 : 'b0;
...
//assign the corresponding clk,aclr,enable and bypass register values.
assign round_clk_pip = clk[round_clkval_pip] ? 'b1 : 'b0;
assign round_aclr_pip = aclr[round_aclrval_pip] || ~devclrn || ~devpor   ? 'b1 : 'b0;
assign round_sload_pip = ena[round_clkval_pip] ? 'b1 : 'b0;
assign round_bypass_register_pip = (round_pipeline_clock == "none") ? 'b1 : 'b0;
...
//assign the corresponding clk,aclr,enable and bypass register values.
assign saturate_clk_pip = clk[saturate_clkval_pip] ? 'b1 : 'b0;
assign saturate_aclr_pip = aclr[saturate_aclrval_pip] || ~devclrn || ~devpor   ? 'b1 : 'b0;
assign saturate_sload_pip = ena[saturate_clkval_pip] ? 'b1 : 'b0;
assign saturate_bypass_register_pip = (saturate_pipeline_clock == "none") ? 'b1 : 'b0;
...
//assign the corresponding clk,aclr,enable and bypass register values.
assign roundchainout_clk_pip = clk[roundchainout_clkval_pip] ? 'b1 : 'b0;
assign roundchainout_aclr_pip = aclr[roundchainout_aclrval_pip] || ~devclrn || ~devpor   ? 'b1 : 'b0;
assign roundchainout_sload_pip = ena[roundchainout_clkval_pip] ? 'b1 : 'b0;
assign roundchainout_bypass_register_pip = (roundchainout_pipeline_clock == "none") ? 'b1 : 'b0;
...
//assign the corresponding clk,aclr,enable and bypass register values.
assign saturatechainout_clk_pip = clk[saturatechainout_clkval_pip] ? 'b1 : 'b0;
assign saturatechainout_aclr_pip = aclr[saturatechainout_aclrval_pip] || ~devclrn || ~devpor   ? 'b1 : 'b0;
assign saturatechainout_sload_pip = ena[saturatechainout_clkval_pip] ? 'b1 : 'b0;
assign saturatechainout_bypass_register_pip = (saturatechainout_pipeline_clock == "none") ? 'b1 : 'b0;
...
//assign the corresponding clk,aclr,enable and bypass register values.
assign fsa0_clk_pip = clk[fsa0_clkval_pip] ? 'b1 : 'b0;
assign fsa0_aclr_pip = aclr[fsa0_aclrval_pip] || ~devclrn || ~devpor   ? 'b1 : 'b0;
assign fsa0_sload_pip = ena[fsa0_clkval_pip] ? 'b1 : 'b0;
assign fsa0_bypass_register_pip = (first_adder0_clock == "none") ? 'b1 : 'b0;
...
//assign the corresponding clk,aclr,enable and bypass register values.
assign fsa1_clk_pip = clk[fsa1_clkval_pip] ? 'b1 : 'b0;
assign fsa1_aclr_pip = aclr[fsa1_aclrval_pip] || ~devclrn || ~devpor   ? 'b1 : 'b0;
assign fsa1_sload_pip = ena[fsa1_clkval_pip] ? 'b1 : 'b0;
assign fsa1_bypass_register_pip = (first_adder1_clock == "none") ? 'b1 : 'b0;
...
assign ssa_datain_width_tmp = (((operation_mode == "accumulator")||(operation_mode == "accumulator_chain_out")||(operation_mode == "two_level_adder_chain_out"))  ? (dataa_width + 8) :
                          (operation_mode == "two_level_adder") ? (dataa_width + 2) :
                          ((operation_mode == "shift" ) || (operation_mode == "36_bit_multiply" )) ? (dataa_width + datab_width):
                          ((operation_mode == "double" )) ? (dataa_width + 8) : dataa_width);

assign ssa_datain_width = (ssa_datain_width_tmp >= round_width) ? ssa_datain_width_tmp : round_width;
...
//assign the corresponding clk,aclr,enable and bypass register values.
assign zeroloopback_clk_or = clk[zeroloopback_clkval_or] ? 'b1 : 'b0;
assign zeroloopback_aclr_or = aclr[zeroloopback_aclrval_or] || ~devclrn || ~devpor   ? 'b1 : 'b0;
assign zeroloopback_sload_or = ena[zeroloopback_clkval_or] ? 'b1 : 'b0;
assign zeroloopback_bypass_register_or = (zeroloopback_output_clock == "none") ? 'b1 : 'b0;
...
//assign the corresponding clk,aclr,enable and bypass register values.
assign zerochainout_clk_or = clk[zerochainout_clkval_or] ? 'b1 : 'b0;
assign zerochainout_aclr_or = aclr[zerochainout_aclrval_or] || ~devclrn || ~devpor   ? 'b1 : 'b0;
assign zerochainout_sload_or = ena[zerochainout_clkval_or] ? 'b1 : 'b0;
assign zerochainout_bypass_register_or = (zerochainout_output_clock == "none") ? 'b1 : 'b0;
...
//assign the corresponding clk,aclr,enable and bypass register values.
assign rs_dataout_clk_or = clk[rs_dataout_clkval_or] ? 'b1 : 'b0;
assign rs_dataout_aclr_or = aclr[rs_dataout_aclrval_or] || ~devclrn || ~devpor   ? 'b1 : 'b0;
assign rs_dataout_sload_or = ena[rs_dataout_clkval_or] ? 'b1 : 'b0;

assign rs_dataout_bypass_register_or_co = (second_adder_clock == "none") ? 'b1 : 'b0;
assign rs_dataout_bypass_register_or_o = (output_clock == "none") ? 'b1 : 'b0;
...
//assign the corresponding clk,aclr,enable and bypass register values.
assign rotate_clk_or = clk[rotate_clkval_or] ? 'b1 : 'b0;
assign rotate_aclr_or = aclr[rotate_aclrval_or] || ~devclrn || ~devpor   ? 'b1 : 'b0;
assign rotate_sload_or = ena[rotate_clkval_or] ? 'b1 : 'b0;
assign rotate_bypass_register_or = (rotate_output_clock == "none") ? 'b1 : 'b0;
...
//assign the corresponding clk,aclr,enable and bypass register values.
assign shiftright_clk_or = clk[shiftright_clkval_or] ? 'b1 : 'b0;
assign shiftright_aclr_or = aclr[shiftright_aclrval_or] || ~devclrn || ~devpor   ? 'b1 : 'b0;
assign shiftright_sload_or = ena[shiftright_clkval_or] ? 'b1 : 'b0;

assign shiftright_bypass_register_or = (shiftright_output_clock == "none") ? 'b1 : 'b0;
...
//assign the corresponding clk,aclr,enable and bypass register values.
assign roundchainout_clk_or = clk[roundchainout_clkval_or] ? 'b1 : 'b0;
assign roundchainout_aclr_or = aclr[roundchainout_aclrval_or] || ~devclrn || ~devpor   ? 'b1 : 'b0;
assign roundchainout_sload_or = ena[roundchainout_clkval_or] ? 'b1 : 'b0;
assign roundchainout_bypass_register_or = (roundchainout_output_clock == "none") ? 'b1 : 'b0;
...
//assign the corresponding clk,aclr,enable and bypass register values.
assign saturatechainout_clk_or = clk[saturatechainout_clkval_or] ? 'b1 : 'b0;
assign saturatechainout_aclr_or = aclr[saturatechainout_aclrval_or] || ~devclrn || ~devpor   ? 'b1 : 'b0;
assign saturatechainout_sload_or = ena[saturatechainout_clkval_or] ? 'b1 : 'b0;
assign saturatechainout_bypass_register_or = (saturatechainout_output_clock == "none") ? 'b1 : 'b0;
...
assign coa_round_width = round_chain_out_width;
assign coa_fraction_width  = coa_datain_width - saturate_chain_out_width;
...
//assign the corresponding clk,aclr,enable and bypass register values.
assign coa_reg_clk_or = clk[coa_reg_clkval_or] ? 'b1 : 'b0;
assign coa_reg_aclr_or = aclr[coa_reg_aclrval_or] || ~devclrn || ~devpor   ? 'b1 : 'b0;
assign coa_reg_sload_or = ena[coa_reg_clkval_or] ? 'b1 : 'b0;
assign coa_reg_bypass_register_or = (output_clock == "none") ? 'b1 : 'b0;
...
assign saturatechainoutoverflow = ((operation_mode == "accumulator_chain_out") ||(operation_mode == "two_level_adder_chain_out"))
                                    ? coa_rs_saturationchainout_overflow_out_reg : 'b0;
...
assign reset = devpor && devclrn && clrn_in && ena_in && rpt_violation;
assign nosloadsclr = reset && (~sload_in && ~sclr_in);
assign sloaddata = reset && sload_in;
...
always @ (asdata_in or clk_in or clrn_in or prn_in or posedge aload_in or 
            devclrn or devpor or posedge violation)
begin
    if (violation == 1'b1)
    begin
...
buf offfsetctrlin_buf[5:0] (offsetdelayctrlin_in,offsetdelayctrlin);
...
begin
    ...
    assign para_static_offset_gray = para_static_offset;
   ...
   assign dr_offsettest_out = (use_offset == "false") ? para_static_offset : offset_in;
 ...
 buf (dqsin_in, dqsin);
 buf delayctrlin_buf [5:0] (delayctrlin_in,delayctrlin);
 buf offsetctrlin_buf[5:0] (offsetctrlin_in,offsetctrlin);
 buf (dqsupdateen_in, dqsupdateen);
 buf phasectrlin_buf [2:0] (phasectrlin_in,phasectrlin);
 ...
 pmos (dqsbusout, tmp_dqsbusout, 'b0);
...
pmos (dqsbusout, tmp_dqsbusout, 'b0);
 ...
 buf       (phaseinvertctrl_in,phaseinvertctrl);
 buf       (dqsenablein_in,dqsenablein);
 buf       (clk_in, clk);
 buf       (enaphasetransferreg_in, enaphasetransferreg);
 buf buf_delayctrl[5:0] (delayctrlin_in,delayctrlin);
 buf buf_phasectrl[3:0] (phasectrlin_in,phasectrlin);
 ...
 assign dataout = tmp_dataout;
  ...
  buf (clk_in, clk);
  buf (phaseselect_in, phaseselect);
  buf buf_delayctrlin [5:0] (delayctrlin_in,delayctrlin);
  buf buf_phasectrlin [3:0] (phasectrlin_in,phasectrlin);
  buf (masterin_in, masterin);
  buf (phaseinvertctrl_in,phaseinvertctrl);
  ...
  buf buf_datain [1:0] (datain_in, datain);
  buf (clk_in, clk);
  buf buf_delayctrl [5:0] (delayctrlin_in, delayctrlin);
  buf buf_phasectrlin [3:0] (phasectrlin_in, phasectrlin);
  buf (areset_in, areset);
  buf (sreset_in, sreset);
  buf (clkena_in, clkena);
  buf (enaoutputcycledelay_in,enaoutputcycledelay);
  buf (enaphasetransferreg_in,enaphasetransferreg);
  buf (phaseinvertctrl_in,phaseinvertctrl);
 ...
 // Duty Cycle Delay
  ...
  buf buf_delayctrlin [5:0] (delayctrlin_in,delayctrlin);
  buf buf_phasectrlin [3:0] (phasectrlin_in,phasectrlin);
  buf (enainputcycledelay_in,enainputcycledelay);
  buf (enaphasetransferreg_in,enaphasetransferreg);
  assign devclrn_in = (devclrn === 1'b0) ? 1'b0 : 1'b1;
  assign devpor_in  = (devpor  === 1'b0) ? 1'b0 : 1'b1;
  ...
  wire        devclrn_in, devpor_in;
  buf buf_datain [1:0]  (datain_ipd, datain);
  buf (directin_in, directin);
  buf (clk_in, clk);
  buf (areset_in, areset);
  buf (dataoutbypass_in, dataoutbypass);
  assign devclrn_in = (devclrn === 1'b0) ? 1'b0 : 1'b1;
  assign devpor_in  = (devpor  === 1'b0) ? 1'b0 : 1'b1;
...
assign rtas_rtoffsetp_in = {"0000", rtsm_rtoffsetp_out[2:0]};
assign rtas_rtoffsetn_in = {"0000", rtsm_rtoffsetn_out[2:0]};
    ...
    begin
       ...
       begin
           ...
           // start reconfiguration
            if (phasecounterselect_ipd < 4'b1100) // no counters selected
           begin
...
assign clk_out_pfd[4] = (pfd_locked == 1'b1) ? clk_tmp[4] : 1'bx;
    ...
    assign clk_out[5] = (test_bypass_lock_detect == "on") ? clk_out_pfd[5] : ((areset_ipd === 1'b1 || pll_in_test_mode === 1'b1) || (locked == 1'b1 && !reconfig_err) ? clk_tmp[5] : 1'bx);
    assign clk_out[6] = (test_bypass_lock_detect == "on") ? clk_out_pfd[6] : ((areset_ipd === 1'b1 || pll_in_test_mode === 1'b1) || (locked == 1'b1 && !reconfig_err) ? clk_tmp[6] : 1'bx);
    assign clk_out[7] = (test_bypass_lock_detect == "on") ? clk_out_pfd[7] : ((areset_ipd === 1'b1 || pll_in_test_mode === 1'b1) || (locked == 1'b1 && !reconfig_err) ? clk_tmp[7] : 1'bx);
    assign clk_out[8] = (test_bypass_lock_detect == "on") ? clk_out_pfd[8] : ((areset_ipd === 1'b1 || pll_in_test_mode === 1'b1) || (locked == 1'b1 && !reconfig_err) ? clk_tmp[8] : 1'bx);
    assign clk_out[9] = (test_bypass_lock_detect == "on") ? clk_out_pfd[9] : ((areset_ipd === 1'b1 || pll_in_test_mode === 1'b1) || (locked == 1'b1 && !reconfig_err) ? clk_tmp[9] : 1'bx);
...
and (mainclk, mainclk_tmp, 'b1);
and (updateclk, updateclk_tmp, 'b1);
and (capture, capture_tmp, 'b1);
and (update, update_tmp, 'b1);
...
//Internal Signals
reg storage_register [0:data_width - 1]; 
reg access_register [0:data_width - 1]; 
reg otpdout_tmp;
reg [data_width - 1:0] mem_init;
reg viol_notifier;
...
begin
    ...
    begin
	    ...
	    begin
            otpdout_tmp <= 1'b0;
            first_use <= 0;               
	        // load data from storage register to access register
	        for ( i = 0; i < data_width; i = i + 1)
