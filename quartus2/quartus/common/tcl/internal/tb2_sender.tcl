...
set QTB_MAX_SECS_ELAPSED_TO_SEND 30
set QTB_MAX_FILES_TO_SEND 15
set STR_NUM_LENGTH 50
set QTB_SENDER_FILE_DELAY 1

# Environment variables
set TB2_SENDER_DEBUG "TB2_SENDER_DEBUG"                         ;# Turn on debug messages for this script
...
set SCHEMA_VERSION "TB2_SCHEMA_VER"                             ;# The schema version of the xsd
...
proc send_file { filename url proxy userpass schemaver } {
	global regtest_mode
	global quartus
	...
	if [file exists $filename] {
        # Schema version for SOPC files is just "SOPCB" (actually it's in the XML,
        # but we don't want to parse it here and Quartus cannot tell us.
        if {[string match "*sopcbuilder*" $filename]} {
            set schemaver "SOPCB"
        }

		set filedata [read_file $filename]

		# Modifications to integrate $filedata into an RFC 1867 package 
		# with Quartus version as another CGI parameter
		
		set rfc1867 {}
		set bound "-----NEXT_PART_[clock seconds].[pid]"
		append rfc1867 "--$bound\r\nContent-Disposition: form-data; name=\"QVER\"\r\n\r\n$quartus(version)\r\n"
		append rfc1867 "--$bound\r\nContent-Disposition: form-data; name=\"SCHEMAVER\"\r\n\r\n$schemaver\r\n"
		...
		if {$userpass != ""} {
			...
			debug_msg "Running http::geturl \"$url\" -type \"multipart/form-data\" -query \$rfc1867 -proxy_auth [concat "Basic" [base64::encode $userpass]]"
			set tok [http::geturl "$url" -type "multipart/form-data; boundary=$bound" -query $rfc1867 -proxy_auth [concat "Basic" [base64::encode $userpass]]]
		} else {
		    debug_msg "Running http::geturl \"$url\" -type \"multipart/form-data\" -query \$rfc1867"
			set tok [http::geturl "$url" -type "multipart/form-data; boundary=$bound" -query $rfc1867]
		}
		upvar #0 $tok data
...
if { [string length $temp_path ] == 0 } {
	...
	if { [string equal $tcl_platform(platform) "unix"] } {
		...
		debug_msg "No temporary directory found in environment.  Looking for: TEMP, TMP, or TMPDIR.  Aborting Quartus Talkback XML file sender."
		exit
	}
}
...
# Check immediately if we in debug mode. That way we can
# just print debug msgs to stdout until we can obtain the
# semaphore and then start writing to the offical debug log
foreach index [array names env $TB2_SENDER_DEBUG] {
    set debug $env($index)
}

#
# Before doing anything, check to see if another tcl_sender is running.  If so, abort.
#

# Semaphores are user-specific, else we may not be able to delete other users' hung semaphonres
set uid $tcl_platform(user)
set semaphore_dir [file join $temp_path "tb2_sender_running_${uid}.sem"]

if [file isdirectory $semaphore_dir ] {
	set t1 [file mtime $semaphore_dir]
	set t2 [clock seconds]
	if { [expr $t2 - $t1] > 120 } {
		# semaphore is old; just delete it and continue
		if [catch {file delete -force $semaphore_dir } err] {
			# If we can't delete it, just exit
			debug_msg $err
			exit
		}
	} else {
		# Semaphore is not old, so abort.
		debug_msg "tb2_sender already running.  Aborting Quartus Talkback XML file sender."
		exit
	}
}

# Okay, we are going to run now, so create the semaphore
file mkdir $semaphore_dir

# Now we can safely open the debug log
set dbg_filename [file join $semaphore_dir "tb2_sender.tcl.out"]
if { $debug } {
	if [catch {set dbg_log [open $dbg_filename w]} err ] {
		debug_msg "Could not open debug output file: $err"
	}
}

debug_msg "\nNow running $argv0\n"
debug_msg "env($TB2_SENDER_DEBUG) == $debug"
...
# We output the temp dir now -- we determined it first thing to create the semaphore
debug_msg "\nTEMP_PATH = $temp_path"
...
if { $regtest_mode } {
	...
	if [catch {set regtest_log [open "tb2_sender.tcl.regtest.out" w]} err ] {
...
set schemaver ""
foreach index [array names env $SCHEMA_VERSION] {
    set schemaver $env($index)
}
...
if {[string length $url] == 0} {
	set url "https://talkback2.altera.com/cgi-bin/talkback2.pl"
}
...
if {[catch {cd $temp_path} err]} {
	...
	set file_count 0
    foreach qtb_file [glob -nocomplain "{tb2,sopcbuilder}_talkback*.xml"] {
		...
		if { [file owned $qtb_file] } {
			...
			while { $attempts < $max_attempts_to_make } {
				if {[send_file [file join $temp_path $qtb_file] $url $proxy $proxy_userpass $schemaver ] == 0} {
...
if { $debug } { 
	close $dbg_log
	set dbg_log 0
	if [catch {file rename -force $dbg_filename $temp_path} err] {
		debug_msg $err
	}
}
if [catch {file delete -force $semaphore_dir} err] {
